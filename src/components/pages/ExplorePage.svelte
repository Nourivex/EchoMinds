<script lang="ts">
  ...
  // Extract unique categories from characters
  const categories = $derived(
    Array.from(new Set(characters.map(c => c.category).filter(Boolean)))
      .map(catId => ({
        id: catId,
  ...
  <div class="max-w-6xl mx-auto">
    ...
    {#if loading}
      ...
      <div class="text-center py-16">
        ...
        <p class="text-slate-600 dark:text-slate-400">Loading characters...</p>
      </div>
    ...
    {:else if error}
      ...
      <div class="text-center py-16">
        ...
        <h3 class="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">Failed to load characters</h3>
